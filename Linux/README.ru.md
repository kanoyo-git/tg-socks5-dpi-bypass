...
3. **TLS ClientHello split** - разрезание первого TLS-пакета на два сегмента. SNI разбивается между сегментами, DPI не может прочитать имя хоста.
...
5. `STATE_TLS_FIRST` → проверка первого пакета на TLS → split если TLS
